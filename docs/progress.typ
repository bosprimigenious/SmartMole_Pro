...
#table(
  ...
  [1], [双板 Wi-Fi 联机对战], [张耀辉], [✓ 已验证],
  [2], [START 同步], [张耀辉], [✓ 已验证],
  [3], [SCORE 同步], [张耀辉], [✓ 已验证],
  [4], [FINISH 同步], [张耀辉], [✓ 已验证],
  [5], [MODE 单机/联机切换], [张耀辉], [✓ 已实现],
  [6], [LEVEL 1–5 闯关], [郭志罡], [✓ 每关固定难度],
  [7], [黄金地鼠 +5 分], [郭志罡], [✓ 已实现],
  [8], [炸弹地鼠 -2 分], [郭志罡], [✓ 已实现],
  [9], [COMBO 连击奖励], [郭志罡], [✓ 已实现],
  [10], [联机联调与双板测试], [朱辰骏/张耀辉], [✓ 双板实机验证],
  [11], [音效框架 + hit.wav], [缪钰], [⚠ 部分（仅 hit 已挂钩）],
  [12], [GPIO LED 击中闪烁], [缪钰], [⚠ 部分（单次 120ms，非 WS2812B）],
  [13], [物理按键 K1], [曹佳轩], [⚠ 部分（触发 START，无 K2）],
  [14], [游戏 UI], [缪钰], [✓ 草地+锤子+9 洞+记分+按钮],
  [15], [WiFi 图形连接], [朱辰骏], [✓ `wifi_ui.c` + wapi],
  [16], [统计数据持久化], [朱辰骏], [⚠ 部分（文件存储 + STATS 弹窗）],
)

#callout(type: "info")[
  联机对战由张耀辉与朱辰骏#strong[分工完成]：张耀辉负责 R528 UDP 框架、versus 协议与 IP/端口配置；朱辰骏参与双板联调与实机测试，提供一块开发板并排查通信与状态同步问题。
]
...
#table(
  ...
  [扩展], [统一事件队列], [×], [曹佳轩], [待开发],
  [进阶], [Wi-Fi 双板联机], [✓], [张耀辉], [双板实机联调，无分屏],
  [进阶], [联机联调与调试], [✓], [朱辰骏/张耀辉], [朱提供对端板，排查 IP/UDP/状态],
  [进阶], [五级闯关], [✓], [郭志罡], [LEVEL 1–5 参数表 L52–58],
  [进阶], [AI 微调难度], [×], [郭志罡], [固定参数表，无 AI 引擎],
  ...
  [工程], [反转模式], [×], [曹佳轩], [未实现],
  [工程], [外壳集成], [×], [缪钰/全员], [答辩前],
  [联机], [IP 可配置], [×], [张耀辉], [硬编码，见 L1103–1105],
)
