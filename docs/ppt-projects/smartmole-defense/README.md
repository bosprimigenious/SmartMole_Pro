...
详见 [`slidev/README.md`](slidev/README.md)
...
| 产物 | 路径 |
|------|------|
| **22 页精美 SVG 源文件** | `svg/01-cover.svg` … `svg/22-closing.svg` |
| **HTML 幻灯片（浏览器演示）** | `html/index.html` |
| **PPT（SVG 栅格化插入）** | `docs/SmartMolePro_答辩PPT_SVG版.pptx` |
...
Windows 无 Cairo 时使用 **Playwright Chromium** 将 SVG 转为 1920×1080 PNG 再写入 PPTX。
...
| 脚本 | 说明 |
|------|------|
| `fetch_ppt_assets.py` + `build_defense_ppt.py` | 照片 + PIL 示意图版 |
| `build_svg_all.py` | **SVG 精排版版（本目录）** |

答辩建议优先使用 **SVG 版 PPT** 或 **HTML 全屏演示**；SVG 源文件可继续在 Figma / Inkscape 中微调。
